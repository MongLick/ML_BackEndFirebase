...
public class SignUpPanel : MonoBehaviour
{
    ...
    public void SignUp()
    {
        ...
        { 
            if(task.IsCanceled)
            {
				panelController.ShowInfo("CreateUserWithEmailAndPasswordAsyne canceled");
				SetInteractable(true);
				return;
			}
            else if (task.IsFaulted)
            {
				panelController.ShowInfo($"CreateUserWithEmailAndPasswordAsyne failed : {task.Exception.Message}");
				SetInteractable(true);
				return;
			}

            panelController.ShowInfo("CreateUserWithEmailAndPasswordAsyne success");
